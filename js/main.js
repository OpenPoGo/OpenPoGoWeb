    ...
    init: function() {
        ...
        $.getScript('https://maps.googleapis.com/maps/api/js?key={0}&libraries=drawing'.format(self.settings.gMapsAPIKey), function() {
            ...
            var user,
                ...
                cells, lastLocation, location, inventory, catchable;
            for (var i = 0; i < self.settings.users.length; i++) {
                user = self.settings.users[i];
                self.user_data[user] = {};
                self.pathcoords[user] = [];
                cells = 'cells-' + mapView.settings.users[i];
                lastLocation = 'last-location-' + mapView.settings.users[i];
                location = 'location-' + mapView.settings.users[i];
                inventory = 'inventory-' + mapView.settings.users[i];
                catchable = 'catchable-' + mapView.settings.users[i];
                ...
                socket.on(lastLocation, function(data) {
                    console.log(data);
                });
                socket.on(location, function(data) {
                    for (var i = 0; i < self.settings.users.length; i++) {
                        self.trainerFunc(data, i);
                    }
                    console.log(data);
                });
                socket.on(inventory, function(data) {
                    for (var i = 0; i < self.settings.users.length; i++) {
                        self.invSuccess(data, i)
                    }
                    console.log(data);
                });
                socket.on(catchable, function(data) {
                    for (var i = 0; i < self.settings.users.length; i++) {
                        self.catchSuccess(data, i);
                    }
                    console.log(data);
                });
            }
